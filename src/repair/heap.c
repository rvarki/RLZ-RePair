...

{
  ...
  int i;
  H.sqrtu = 2;
  while ((relong)H.sqrtu * (relong)H.sqrtu < u)
    H.sqrtu++;
  H.infreq = (void *)malloc(H.sqrtu * sizeof(Tarray));
  for (i = 1; i < H.sqrtu; i++)
    H.infreq[i] = createArray(Rec, factor, minsize);
  H.freq = malloc(H.sqrtu * sizeof(Thnode));
  H.freef = 0;
  for (i = 0; i < H.sqrtu - 1; i++)
    H.freq[i].next = i + 1;
...
{ // Trecord *rec = H->Rec->records;
  ...
  if (PRNH)
    prnH(H);
  if ((H->max == H->sqrtu) && (H->largest == -1))
    H->max--;
  if (H->max < H->sqrtu)
  {
    while (H->max && (H->infreq[H->max].size == 0))
      H->max--;
    if (!H->max)
      return -1; // empty heap
